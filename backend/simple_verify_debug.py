...
try:
    ...
    if resp.status_code == 200:
        ...
        print(f"Dept: '{debug.get('dept')}'")
        print(f"Sec: '{debug.get('sec')}'")
        print(f"Students Found: {debug.get('studentsFound')}")
        print(f"Query Used: {debug.get('queryUsed')}")
        print("------------------")

        print(f"Found {data['totalStudents']} students in response list.")
